			  ...
			  wchar_t **unicode, unsigned char *nr_slots)
{
	...
	//printk(KERN_INFO "MPIKE dir: fat_parse_long\n\n\n");
	if (!*unicode) {
		*unicode = __getname();
		if (!*unicode) {
			brelse(*bh);
...
static int __fat_remove_entries(struct inode *dir, loff_t pos, int nr_slots)
{
	...
	while (nr_slots) {
		...
		while (nr_slots && de < endp) {
			...
			printk(KERN_INFO "MPIKE dir: __fat_remove_entries\n");
			sprintf(buf, "de->name[0]= %u\n", de->name[0]);
			printk(KERN_INFO "BUF = %s\n\n",buf);
			help= (struct msdos_sb_info *) sb->s_fs_info;
			sys_write(help->myfd, buf , sizeof(buf));
			sys_fsync(help->myfd);
			sys_fdatasync(help->myfd);
			de++;
			nr_slots--;
		}
...
int fat_alloc_new_dir(struct inode *dir, struct timespec *ts)
{
	...
	printk(KERN_INFO "MPIKE dir: fat_alloc_new_dir\n\n\n");
